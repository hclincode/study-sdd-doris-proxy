...
- Q: When a statement already specifies its own row limit, how should the proxy's 200-row cap interact with it? → A: Apply the smaller of the user's limit and 200; the offset is applied first and is never altered.
- Q: For statements that write rows as a result of an inner query, such as `INSERT INTO ... SELECT`, should the proxy cap the inner query, leave the statement alone, or reject it? → A: Leave it alone and record a distinct reason; never cap and never reject.
- Q: When the proxy truncates a result the user did not ask to truncate, should the client be able to tell? → A: Yes — a retrievable advisory attached to the statement's response, which does not alter the rows or columns returned.
...
2. **Given** a table with 10,000 rows, **When** the analyst submits `SELECT * FROM events LIMIT 500`,
   **Then** the client receives 200 rows and can retrieve an advisory saying the result was truncated.
...
- What happens when the statement is a write that carries a query inside it, such as
  `INSERT INTO summary SELECT ... FROM events`? Resolved by FR-007: forwarded unchanged with its own
  recorded reason, because capping it would silently write fewer rows than intended.
...
- **FR-007**: For statements that write rows as a result of an inner query, such as
  `INSERT INTO ... SELECT` or `CREATE TABLE ... AS SELECT`, System MUST forward the statement unchanged
  and MUST record a reason distinct from both "could not be parsed" and "no safe position". System MUST
  NOT cap such a statement and MUST NOT reject it.
- **FR-008**: When a result has been truncated because of the proxy's cap rather than because of the
  user's own row count, System MUST attach an advisory to that statement's response which the client can
  retrieve, stating that the result was truncated and at what row count. The advisory MUST NOT change
  the rows returned, the number of rows returned, or the columns returned.
...
- **SC-001**: No statement whose cap decision is anything other than "forwarded unchanged" returns more
  than 200 rows to the client, including statements that carried a larger row count of their own.
...
- **SC-008**: 100% of statements whose results were truncated by the cap carry a retrievable advisory
  saying so, and 0% of statements whose results were not truncated carry one.
...
- Analysts who need complete results reach Doris by a route that does not pass through this proxy. Which
  route that is, and who is entitled to it, is an operational decision outside this feature. Writing a
  larger row count in the statement is explicitly not that route (FR-006, FR-014).
