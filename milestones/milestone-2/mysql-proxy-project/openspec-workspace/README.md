...
| **`GROUP BY … WITH ROLLUP` is refused — at no cost** | `sqlparser`'s `MySqlDialect` cannot parse it, so the proxy refuses it. **Verified against Doris 2.1: Doris cannot parse it either** (`ParseException … no viable alternative at input 'WITH ROLLUP'`), so the statement would have failed at the backend regardless. The proxy's error arrives earlier and reads better. Listed because it looks like a compatibility gap and is not — it was recorded as this project's largest concrete compatibility cost until a real backend was consulted. |
| **`GROUP BY ALL` is refused** | Refused deliberately rather than by parser gap. `sqlparser` accepts it as a distinct construct; MySQL treats `ALL` as reserved and rejects the statement, and Doris follows MySQL. The two disagree about what the text *is*, which is the parser differential in its purest form, so the allowlist declines it. Costs nothing that works today — these already fail at the backend — but the error now comes from the proxy and says why. |
