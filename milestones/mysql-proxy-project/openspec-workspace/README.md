...
| **`GROUP BY … WITH ROLLUP` is refused** | `sqlparser`'s `MySqlDialect` cannot parse it **at all** — not an exotic variant, the plain form. It is ordinary MySQL and common in reporting queries, so any policy-bearing user issuing one is refused. This is the largest concrete instance of the compatibility cost above: the abstract "no Doris dialect" warning, priced. There is no workaround inside the proxy; rewrite the query with explicit aggregation, or exempt the user from policy. |
